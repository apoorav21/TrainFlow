...
Uses gpt-5.4 (128k output tokens) to generate the complete training schedule in a
single API call, then batch-writes all days to DynamoDB.
"""
...
def handler(event, context):
    ...
    print(f'[plan/generate] Generating {total_weeks}-week plan {plan_id} in one shot using gpt-5.4')
    ...
    prompt = (
        f"Generate the complete {total_weeks}-week {plan_meta.get('goalType', 'running')} training plan.\n"
        f"Plan starts: {start_date}. Training days per week: {days_per_week}.\n"
        f"User context: {user_context}\n\n"
        f"Return exactly {total_days} day objects (weeks 1–{total_weeks}, days 1–7 each). "
    ...
    max_output = min(total_days * 400 + 8000, 100000)

    for attempt in range(3):
        try:
            print(f'[plan/generate] Attempt {attempt + 1}: requesting {total_days} days')
            ...
            print(f'[plan/generate] Received {len(all_days)} days from gpt-5.4')
            break
